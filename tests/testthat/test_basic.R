...
test_that("Precision is well-behaved", {
  ...
  sanity(tmp) ; expect_equal(tmp$metric, 0.2) ; expect_equal(tmp$ETU, 1)
...
test_that("RBP is well-behaved", {
  tmp <- RBP(c(1), p=.6)
  sanity(tmp) ; expect_equal(tmp$metric, .4) ; expect_equal(tmp$metric + tmp$residual, 1) ; expect_equal(tmp$ETU, 1)
...
test_that("SDCG is well-behaved", {
  ...
  sanity(tmp) ; expect_equal(tmp$metric, 1) ; expect_equal(tmp$ETU, 1)
  tmp <- SDCG(c(1, 0), k=2)
  sanity(tmp) ; expect_equal(tmp$metric, .613, tolerance=0.001) ; expect_equal(tmp$ETU, 1)
  tmp <- SDCG(c(0), k=1)
  sanity(tmp) ; expect_equal(tmp$metric, 0) ; expect_equal(tmp$ETU, 0)

  tmp <- SDCG(c(1, 0, 0, 0, 0), k=5)
  sanity(tmp) ; expect_equal(tmp$metric, .339, tolerance=0.001) ; expect_equal(tmp$ETU, 1)
  tmp <- SDCG(c(0, 0, 0, 0, 1), k=5)
  sanity(tmp) ; expect_equal(tmp$metric, .131, tolerance=0.001) ; expect_equal(tmp$ETU, .387, tolerance=0.001)
